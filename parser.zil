...
<ROUTINE PARSER ("AUX" (PTR ,P-LEXSTART) WRD (VAL 0) (VERB <>) (BOTTOM <>)
	...
	<COND (,RESERVE-PTR
	      ...
	      (,P-CONT
	       <SET PTR ,P-CONT>
	       <COND (<AND <EQUAL? ,PROTAGONIST ,WINNER>
			   ;<NOT <VERB? TELL>>
			   <NOT ,SUPER-BRIEF>>
		      <CRLF>)>)
	      (T
...
"It disappeared before you could get a good look." CR>)
		...
		<RTRUE>)
	       (<AND <ZERO? ,FUZZY?>
		     <EQUAL? .WORD ,W?FUZZY ,W?BLURRY ,W?BLURRED>>
...
<ROUTINE BUT-MERGE (TBL "AUX" LEN BUTLEN (CNT 1) (MATCHES 0) OBJ NTBL) 
	...
	<REPEAT ()
		<COND (<DLESS? LEN 0> <RETURN>)
		      (<INTBL? <SET OBJ <GET .TBL .CNT>> <REST ,P-BUTS 2> <GET ,P-BUTS 0>>)
...
<ROUTINE SNARFEM (PTR EPTR TBL "AUX" (BUT <>) LEN WV WRD NW (WAS-ALL? <>)) 
   ;"start with a clean slate -- pdl 7/5/88"
   <SETG P-NAM <>>
   <SETG P-ADJ <>>
...
<ROUTINE GET-OBJECT (TBL "OPTIONAL" (VRB T)
		    "AUX" BITS LEN XBITS TLEN (GCHECK <>) (OLEN 0) OBJ ADJ X)
...
<ROUTINE MOBY-FIND (TBL "AUX" OBJ LEN FOO)
         <SETG P-NAM ,P-XNAM>
         <SETG P-ADJ ,P-XADJ>
         <PUT .TBL ,P-MATCHLEN 0>
         <SET OBJ 1>
	 <REPEAT ()
	    <SET FOO <META-LOC .OBJ>>
	    <COND (.FOO
		   <SET FOO <THIS-IT? .OBJ>>
		   <COND (.FOO
...
<ROUTINE GLOBAL-CHECK (TBL "AUX" LEN RMG RMGL CNT OBJ OBITS FOO) 
	<SET LEN <GET .TBL ,P-MATCHLEN>>
	...
	<COND (.RMG
		  ...
		  <COND (<EQUAL? ,P-NAM <GET .RMG <* .CNT 2>>>
			 <PUTP ,PSEUDO-OBJECT ,P?ACTION
			       <GET .RMG <+ <* .CNT 2> 1>>>
			 <SET FOO <BACK <GETPT ,PSEUDO-OBJECT ,P?ACTION> 7>>
			 <COPYT ,P-NAM .FOO 6>
...
<ROUTINE SEARCH-LIST (OBJ TBL LVL "AUX" NXT)
...
<ROUTINE THIS-IT? (OBJ "AUX" SYNS)
	 ...
	 <COND (<ZERO? .SYNS>
		...
		<RFALSE>)
	       (,P-ADJ
		<SET SYNS <GETPT .OBJ ,P?ADJECTIVE>>
		<COND (<ZERO? .SYNS>
		       <RFALSE>)
 ...
 <COND (<FSET? .OBJ ,INVISIBLE>
	<RFALSE>)
       (<AND ,P-NAM
		 ...
		 <NOT <INTBL? ,P-NAM .SYNS <- </ <PTSIZE .SYNS> 2> 1>>>>>
	<RFALSE>)
       (<AND ,P-ADJ
